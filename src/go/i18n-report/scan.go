...
var (
	...
	// Dynamic key prefix pattern: `some.prefix.${...}`
	dynPrefixPattern = regexp.MustCompile("(?:`|['\"])([a-zA-Z0-9_.]+)\\.\\$\\{")
)
...
// dynamicKeyPrefixes returns key prefixes that are constructed dynamically
// in source code (e.g., `locale.${locale}` makes "locale." dynamic).
func dynamicKeyPrefixes(root string) ([]string, error) {
	...
	for _, file := range files {
		...
		matches := dynPrefixPattern.FindAllStringSubmatch(string(data), -1)
