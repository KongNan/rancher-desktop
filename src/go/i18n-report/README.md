...
Keys under dynamic prefixes (e.g., `locale.${locale}`) are excluded
automatically.
...
List used keys missing from a locale, with their English values. This
filters out the thousands of unused keys inherited from @rancher/components,
showing only keys that Rancher Desktop displays.
...
Run unused, stale, and translate checks together. Reports pass/fail counts
...
```
  ...
  used keys missing from de:      0  OK
All checks passed.
```
...
```sh
i18n-report unused                  # find unreferenced keys
# Remove them from en-us.yaml and all locale files.
i18n-report stale --locale=de       # confirm no orphans remain
```
...
- Dynamic prefixes: template literals like `` `locale.${locale}` ``
