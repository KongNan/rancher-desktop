...
func TestDynPrefixPattern(t *testing.T) {
	tests := []struct {
		name       string
		line       string
		wantPrefix string
	}{
		{"backtick template", "`locale.${locale}`", "locale"},
		{"single quote prefix", "'prefix.${var}'", "prefix"},
		{"no dynamic", "'static.key'", ""},
	}

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			m := dynPrefixPattern.FindStringSubmatch(tc.line)
			var found string
			if m != nil {
				found = m[1]
			}
			if tc.wantPrefix == "" && found != "" {
				t.Errorf("expected no match, got %q", found)
			} else if tc.wantPrefix != "" && found != tc.wantPrefix {
				t.Errorf("got %q, want %q", found, tc.wantPrefix)
			}
		})
	}
}
