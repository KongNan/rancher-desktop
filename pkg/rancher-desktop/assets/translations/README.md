...
A Go CLI at `src/go/i18n-report/` with seven report subcommands and one
action subcommand. See `src/go/i18n-report/README.md` for full documentation.
...
| `translate` | Used keys missing from a locale, with English values |
...
| `check` | Combined lint check (unused + stale + translate) |
...
1. Run `i18n-report unused` to find dead keys. Remove them from en-us.yaml
   and all locale files.
2. Run `i18n-report stale --locale=<code>` to find keys that exist in a
   locale file but not in en-us.yaml. Remove them.
3. Run `i18n-report translate --locale=<code>` to find used keys that need
   translation (already filtered to exclude unused keys). Use
   `prompt-update-translations.md` to fill them in.
