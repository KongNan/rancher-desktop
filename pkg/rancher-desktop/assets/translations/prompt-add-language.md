...
Run the translate report to get all used keys with their English values:

```sh
go tool i18n-report translate --locale=<code>
```

This filters out unused keys inherited from @rancher/components, showing only
keys that Rancher Desktop displays. Each line is `key=value`.
